...
%% Number of robots
function [L,energy] = rendezvous_experiments(N,algorithm,iterations)
...
desired_distance = 0.5; % desired inter-agent distance range to realize stop condition
...
desired_energy =  10; % (currently not used) desired value of the Lyapunov candidate energy function
...
si_to_uni_dyn = create_si_to_uni_dynamics_with_backwards_motion('LinearVelocityGain', linearVelocityGain, 'AngularVelocityLimit', angularVelocityGain);
...
max_iterations = iterations; % the number of iterations for the experiment
...
for t = 1:max_iterations
    ...
    if ((stop_condition == 1) && (iteration_at_stopcondition == 0))
        fprintf('Stop condition (all inter-robot distances within 0.5m) reached at %d',t);
        iteration_at_stopcondition = t;
    end
    
    %if((energy(t) <= desired_energy) && (iteration_at_minenergy == 0))
        %fprintf('Minimum energy condition (E<=10) reached at %d',t);
    %end
    %% Movie making
    if(make_movie == 1)
        set(0,'CurrentFigure',r.figure_handle);
